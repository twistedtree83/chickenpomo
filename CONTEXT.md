...
6. **Sprite loading fails gracefully — but the fallback never reaches the user.** Each renderer still falls back to a coloured rectangle when its sprite handle is `null` (so headless tests, the preview harness, and pre-art development all work). At runtime, `Scene` holds the rAF loop until `Modules.assetsReady` resolves, so users see the page's sky-coloured backdrop through the transparent canvas instead of a flash of fallback bars. Without this gate any code path that rebuilds modules mid-session (HMR, future remount) would briefly paint the fallbacks.
